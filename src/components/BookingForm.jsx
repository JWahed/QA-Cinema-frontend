...
const BookingDetails = () => {
  ...
  const [countConcession, setCountConcession] = useState(0);
  const ticketAndFoodPrice = [10.75, 7.25, 5.65];

  let totalPrice = Number(
    ticketAndFoodPrice[0] * countAdult +
      ticketAndFoodPrice[1] * countChild +
      ticketAndFoodPrice[2] * countConcession
  ).toFixed(2);
  let countAll = countAdult + countChild + countConcession;
  ...
  const setConcessionUp = (event) => {
    event.preventDefault();
    setCountConcession((countConcession) => Math.min(24, countConcession + 1));
  };

  const setConcessionDown = (event) => {
    event.preventDefault();
    setCountConcession((countConcession) => Math.max(0, countConcession - 1));
  };
  ...
  return (
    <div onSubmit={submitForm}>
      ...
      <select
        ...
        aria-label="Default select example"
      >
        <option selected>Select a Time and Screen</option>
      ...
      <br />
      <br />
      <label>Adult Seats (£{ticketAndFoodPrice[0]} each) </label>
      ...
      <br />
      <br />
      <label>Child Seats (£{ticketAndFoodPrice[1]} each) </label>
      ...
      <br />
      <br />
      <label>Popcorn (£{ticketAndFoodPrice[2]} each) </label>
      <button id="neg" onClick={setConcessionDown}>
        -
      </button>
      <button id="pos" onClick={setConcessionUp}>
        +
      </button>
      <br />
      <input id="seatCounter" placeholder={countConcession} disabled></input>
